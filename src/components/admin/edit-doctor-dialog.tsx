...
const schema = z
  .object({
    ...
    qualifications: z.string().min(2, "Qualifications are required").optional(),
  ...
  .superRefine((values, context) => {
    if (values.location && values.location !== "Other" && values.state && !NIGERIAN_STATES.includes(values.state as (typeof NIGERIAN_STATES)[number])) {
      context.addIssue({
...
export function EditDoctorDialog({ open, onOpenChange, doctor, onSuccess }: EditDoctorDialogProps) {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      location: (doctor?.location || "Abuja") as any,
      state: doctor?.state || "FCT",
      bio: doctor?.bio || "",
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      if (axiosErr.response?.status === 409) {
        setError("A doctor with this email or phone already exists.");
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Update doctor information and profile photo. Leave fields blank to keep existing values unchanged.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          ...
          {/* Location callout */}
          <div className="flex items-start gap-3 rounded-lg border border-blue-200 bg-blue-50 dark:border-blue-800/50 dark:bg-blue-900/20 p-3">
            <MapPin className="mt-0.5 h-4 w-4 shrink-0 text-blue-600 dark:text-blue-400" />
            <p className="text-sm text-blue-800 dark:text-blue-300">
              Update doctor location and facility assignment as needed.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Select
                value={selectedLocation || (doctor?.location || "Abuja")}
            ...
            <div className="space-y-2">
              ...
              {selectedLocation === "Other" ? (
                ...
                <Select
                  value={form.getValues("state") || (doctor?.state || "FCT")}
            ...
            <div className="rounded-md border border-border bg-muted/20 p-3">
              <div className="mb-3 flex items-center gap-3">
                <div className="flex h-16 w-16 items-center justify-center overflow-hidden rounded-full border bg-muted">
                  {uploadedAvatarUrl || doctor?.avatar ? (
                    <img src={uploadedAvatarUrl || doctor?.avatar} alt="Doctor photo preview" className="h-full w-full object-cover" />
                  ) : (
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                {(uploadedAvatarUrl || doctor?.avatar) && !isUploadingPhoto && !photoUploadError && (
                  <span className="inline-flex items-center gap-1 text-xs text-green-600">
                    <CheckCircle2 className="h-3.5 w-3.5" /> Photo ready
                  </span>
                )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input
                id="name"
                placeholder={doctor?.name || "Dr. John Doe"}
                {...form.register("name")}
                disabled={isSubmitting}
              />
            ...
            <div className="space-y-2">
              ...
              <Input
                id="email"
                type="email"
                placeholder={doctor?.email || "doctor@example.com"}
                {...form.register("email")}
                disabled={isSubmitting}
              />
          ...
          <div className="space-y-2">
            <div className="space-y-2">
              ...
              <Input
                id="phone"
                type="tel"
                placeholder={doctor?.phone || "+234 xxx xxx xxxx"}
                {...form.register("phone")}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input
                id="qualifications"
                placeholder={doctor?.qualifications || "MBBS, FMCGP"}
            ...
            <div className="space-y-2">
              ...
              <Select
                value={form.getValues("specialty") || (doctor?.specialty || "General Practice")}
          ...
          <div className="space-y-2">
            ...
            <Textarea
              id="bio"
              placeholder={doctor?.bio || "A short biography that will appear on the public doctor profile page..."}
              rows={5}
              maxLength={2000}
              {...form.register("bio")}
              disabled={isSubmitting}
