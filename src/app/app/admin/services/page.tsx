...
export default function AdminServicesPage() {
  ...
  const openCreate = () => {
    setEditingId(null);
    setEditingLocation(adminLocation);
    setForm(emptyForm());
    setKsTitle("");
    setKsDesc("");
    setDialogOpen(true);
  ...
  const openEdit = (svc: Service) => {
    setEditingId(svc.id);
    setEditingLocation(svc.location);
    setForm({
      name: svc.name,
  ...
  const addKeyService = () => {
    ...
    setForm((prev) => ({
      ...
      keyServices: [
        ...prev.keyServices,
        { id: `ks-new-${Date.now()}`, title: ksTitle.trim(), description: ksDesc.trim() },
      ],
    }));
    setKsTitle("");
    setKsDesc("");
                ...
                <div className="relative flex h-32 cursor-pointer items-center justify-center overflow-hidden rounded-lg border-2 border-dashed border-border hover:border-primary/50 transition-colors bg-muted/20" onClick={() => bannerInputRef.current?.click()}>
                  {form.bannerImageUrl ? (
                    <>
                      ...
                      <button type="button" className="absolute right-1 top-1 rounded-full bg-black/50 p-0.5 text-white hover:bg-black/70" onClick={(e) => { e.stopPropagation(); setForm((p) => ({ ...p, bannerImageUrl: undefined })); }}>
                        <X className="h-3 w-3" />
                      </button>
                    </>
                  ) : (
                ...
                <div className="relative flex h-32 cursor-pointer items-center justify-center overflow-hidden rounded-lg border-2 border-dashed border-border hover:border-primary/50 transition-colors bg-muted/20" onClick={() => iconInputRef.current?.click()}>
                  {form.iconImageUrl ? (
                    <>
                      ...
                      <button type="button" className="absolute right-1 top-1 rounded-full bg-black/50 p-0.5 text-white hover:bg-black/70" onClick={(e) => { e.stopPropagation(); setForm((p) => ({ ...p, iconImageUrl: undefined })); }}>
                        <X className="h-3 w-3" />
                      </button>
                    </>
                  ) : (
                ...
                <div className="space-y-2 rounded-lg border border-border/60 p-3">
                  {form.keyServices.map((ks, i) => (
                    <div key={ks.id} className="flex items-start gap-3 group">
                      <div className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-green-500 to-lime-600 text-xs font-bold text-white">{i + 1}</div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium">{ks.title}</p>
                        {ks.description && <p className="text-xs text-muted-foreground">{ks.description}</p>}
                      </div>
                      <Button size="icon" variant="ghost" className="h-6 w-6 shrink-0 opacity-0 group-hover:opacity-100 text-destructive hover:text-destructive" onClick={() => removeKeyService(ks.id)}>
...
// â”€â”€ Reusable service card â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function ServiceCard({
  ...
  return (
    <Card className={`overflow-hidden border-border/60 ${!canEdit ? "opacity-70" : ""}`}>
      <div className="relative h-36 bg-gradient-to-br from-green-500/20 to-lime-500/10 overflow-hidden">
        ...
        {svc.iconImageUrl && (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={svc.iconImageUrl} alt="Icon" className="absolute bottom-2 right-2 h-10 w-10 rounded-full border-2 border-white dark:border-slate-800 object-cover bg-white shadow" />
        )}
        {/* Location tag */}
        <div className="absolute left-2 top-2 flex items-center gap-1 rounded-full bg-black/50 px-2 py-0.5 text-xs font-medium text-white backdrop-blur-sm">
        ...
        <div className="flex items-start justify-between gap-2">
          ...
          {canEdit ? (
            <div className="flex gap-1 shrink-0">
              <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => onEdit(svc)}>
                <Pencil className="h-3.5 w-3.5" />
              </Button>
              <Button size="icon" variant="ghost" className="h-7 w-7 text-destructive hover:text-destructive" onClick={() => onDelete(svc.id)}>
